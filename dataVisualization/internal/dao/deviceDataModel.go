...
type Extension interface {
	...
	GetDeviceDataByColumns(ctx context.Context, params *query.Params, tableName string) ([]*model.DeviceDataModel, int64, error)
	CountDeviceDataByColumns(ctx context.Context, params *query.Params, tableName string) (count int64, err error)
}
...
func (d *deviceDataModelDao) GetDeviceData(ctx context.Context, device *model.Device) ([]*model.DeviceDataModel, error) {
	...
	table := []*model.DeviceDataModel{}
	err := d.db.WithContext(ctx).Table(device.Code).Find(&table).Error
	if err != nil {
		return nil, err
	}
...
func (d *deviceDataModelDao) GetDeviceDataByColumns(ctx context.Context, params *query.Params, tableName string) ([]*model.DeviceDataModel, int64, error) {
	queryStr, args, err := params.ConvertToGormConditions()
	if err != nil {
		return nil, 0, errors.New("query params error: " + err.Error())
	}
	...
	var total int64
	if params.Sort != "ignore count" { // determine if count is required
		err = d.db.WithContext(ctx).Table(tableName).Select([]string{"id"}).Where(queryStr, args...).Count(&total).Error
		if err != nil {
			return nil, 0, err
		}
		if total == 0 {
	...
	records := []*model.DeviceDataModel{}
	order, limit, offset := params.ConvertToPage()
	err = d.db.WithContext(ctx).Table(tableName).Order(order).Limit(limit).Offset(offset).Where(queryStr, args...).Find(&records).Error
	if err != nil {
		return nil, 0, err
	}
...
func (d *deviceDataModelDao) CountDeviceDataByColumns(ctx context.Context, params *query.Params, tableName string) (count int64, err error) {

	queryStr, args, err := params.ConvertToGormConditions()
	if err != nil {
		return 0, errors.New("query params error: " + err.Error())
	}

	err = d.db.WithContext(ctx).Table(tableName).Select([]string{"id"}).Where(queryStr, args...).Count(&count).Error
	if err != nil {
		return 0, err
	}
