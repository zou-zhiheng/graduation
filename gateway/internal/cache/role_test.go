...
func newRoleCache() *gotest.Cache {
	...
	c.ICache = NewRoleCache(&model2.CacheType{
		CType: "redis",
		Rdb:   c.RedisClient,
	})
	return c
...
func Test_roleCache_Set(t *testing.T) {
	...
	record := c.TestDataSlice[0].(*model2.Role)
...
func Test_roleCache_MultiGet(t *testing.T) {
	...
	var testData []*model2.Role
	for _, data := range c.TestDataSlice {
		testData = append(testData, data.(*model2.Role))
	}
	...
	for k, v := range expected {
		assert.Equal(t, got[utils.StrToUint64(k)], v.(*model2.Role))
	}
}
...
func TestNewRoleCache(t *testing.T) {
	c := NewRoleCache(&model2.CacheType{
		CType: "",
	})
	assert.Nil(t, c)
	c = NewRoleCache(&model2.CacheType{
