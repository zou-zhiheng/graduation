...
// NewManagerRPCConn instantiate rpc client connection
func NewManagerRPCConn() {
	cfg := config.Get()
	...
	var err error
	ManagerConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
...
// GetManagerRPCConn get client conn
func GetManagerRPCConn() *grpc.ClientConn {
	if ManagerConn == nil {
		ManagerOnce.Do(func() {
			NewManagerRPCConn()
		})
...
// CloseManagerRPCConn Close tears down the ClientConn and all underlying connections.
func CloseManagerRPCConn() error {
	if ManagerConn == nil {
		return nil
	}

	return ManagerConn.Close()
}
